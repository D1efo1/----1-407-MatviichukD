...
def find_conf_matrix_values(y_true, y_pred):
    ...
    TP = find_TP_Matviichuk(y_true, y_pred)
    FN = find_FN_Matviichuk(y_true, y_pred)
    FP = find_FP_Matviichuk(y_true, y_pred)
    TN = find_TN_Matviichuk(y_true, y_pred)
    return TP, FN, FP, TN

# Функція для створення матриці помилок
def matviychuk_confusion_matrix(y_true, y_pred):
...
assert np.array_equal(matviychuk_confusion_matrix(df.actual_label.values, df.predicted_RF.values),
                      confusion_matrix(df.actual_label.values, df.predicted_RF.values)), 'matviychuk_confusion_matrix() is not correct for RF'
assert np.array_equal(matviychuk_confusion_matrix(df.actual_label.values, df.predicted_LR.values),
                      confusion_matrix(df.actual_label.values, df.predicted_LR.values)), 'matviychuk_confusion_matrix() is not correct for LR'

# Використання функцій для підрахунку TP, FN, FP та TN
TP_RF = find_TP_Matviichuk(df['actual_label'].values, df['predicted_RF'].values)
FN_RF = find_FN_Matviichuk(df['actual_label'].values, df['predicted_RF'].values)
FP_RF = find_FP_Matviichuk(df['actual_label'].values, df['predicted_RF'].values)
TN_RF = find_TN_Matviichuk(df['actual_label'].values, df['predicted_RF'].values)
